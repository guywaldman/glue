...
mod tests {
    ...
    #[test]
    fn generate_to_output_overwrites_existing_file_even_when_body_is_unchanged() {
        let temp_dir = unique_temp_dir("overwrite_existing_output");
        std::fs::create_dir_all(&temp_dir).expect("temp dir should be created");

        let source_path = temp_dir.join("user.glue");
        let output_path = temp_dir.join("generated").join("user.ts");
        let source_contents = "model User { id: string }\n";
        std::fs::write(&source_path, source_contents).expect("source should be written");
        std::fs::create_dir_all(output_path.parent().expect("output should have parent")).expect("output dir should be created");
        ...
        let generated_body = CodeGen::generate(CodeGenMode::TypeScript, &source, None).expect("typescript should generate");
        std::fs::write(
            &output_path,
            format!(
                "// {}\n// stale watermark that should be overwritten\n// {}\n\n{}",
                GlueCli::WATERMARK_SEPERATOR,
                GlueCli::WATERMARK_SEPERATOR,
                generated_body
            ),
        )
        ...
        assert!(output.contains("// Generated by Glue on "));
        assert!(!output.contains("stale watermark that should be overwritten"));
