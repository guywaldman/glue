...
fn main() -> miette::Result<()> {
    ...
    // Lex
    let analyzer = gluelang::Analyzer::new("sample.rs", src);
    let result = analyzer.analyze();

    let program = match result {
        Ok(program) => program,
        Err(AnalyzerError::SemanticErrors(errs)) => {
            eprintln!("[ERROR] Found {} semantic errors:", errs.len());
            ...
            return Err(miette::Report::new(errs.into_iter().next().unwrap()));
        }
        _ => {
            eprintln!("[ERROR] Failed to analyze program: {result:?}");
            return Err(miette::miette!("Failed to analyze program"));
        }
    };
