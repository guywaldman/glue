...
Set `serde_struct_derives: false` to omit serde `Serialize`/`Deserialize` derives and serde field attributes from generated structs.
...
- Unions are emitted as generated `#[serde(untagged)]` enums.
