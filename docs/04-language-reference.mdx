...
Glue supports top-level constants for reusable literal values.
...
- Names should use `CONSTANT_CASE`. A leading `_` marks a constant as private in generated language targets.
- Constants can be used in field defaults and decorator arguments.
- TypeScript, Python, Rust, and Go emit standalone constants. OpenAPI, JSON Schema, and Protobuf only consume folded constant values.
...
model User {
  @field(alias=USER_EMAIL_ALIAS, example="user@example.com")
