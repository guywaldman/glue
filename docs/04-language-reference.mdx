...
Model, anonymous model, endpoint, service, and RPC blocks accept either whitespace/newlines or commas between members.
Commas are useful for compact one-line shapes:

```glue
model User { id: string, email?: string, profile: { name: string, age?: u8 } }

endpoint "GET /users/{id}" GetUser { responses: { 200: User, 4XX: ApiError } }
```
